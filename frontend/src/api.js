...
export async function getNotificaciones(usuario_id) {
  const response = await fetch(`${API_URL}/notificaciones/${usuario_id}`);
...
export async function getMensajes(conversacionId) {
  const response = await fetch(`${API_URL}/mensajes/${conversacionId}`);
...
export async function getMensajesRecibidos(destinatarioId) {
  const response = await fetch(`${API_URL}/mensajes/recibidos/${destinatarioId}`);
